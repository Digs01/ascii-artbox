...
exports.createCompositeAsciiVideo = createCompositeAsciiVideo;
...
async function createCompositeAsciiVideo(layers, options) {
    const tempDir = path_1.default.join(process.cwd(), `temp-ascii-comp-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`);
    await mkdir(tempDir, { recursive: true });
    const outputPath = path_1.default.join(tempDir, 'output.mp4');
    try {
        ...
        for (let i = 0; i < totalFrames; i += chunkSize) {
            ...
            await Promise.all(indices.map(async (frameIndex) => {
                ...
                let compositor = (0, sharp_1.default)({
                    create: {
                        width,
                        height,
                        channels: 4,
                ...
                for (const layer of layers) {
                    ...
                    let layerImg = (0, sharp_1.default)(buffer);
                    // 1. Flip
                    if (flipX)
                        layerImg = layerImg.flop();
                    if (flipY)
                        layerImg = layerImg.flip();
                    // 2. Scale
                    if (scale !== 1) {
                    ...
                    const tMeta = await (0, sharp_1.default)(transformedBuffer).metadata();
                    ...
                    layerBuffers.push({
                        ...
                        top,
                        left,
                        blend: layer.transform.blendMode === 'normal' ? 'over' : layer.transform.blendMode,
                ...
                await writeFile(path_1.default.join(tempDir, `frame-${String(frameIndex).padStart(5, '0')}.png`), finalFrame);
            }));
        }
        await new Promise((resolve, reject) => {
            (0, fluent_ffmpeg_1.default)()
