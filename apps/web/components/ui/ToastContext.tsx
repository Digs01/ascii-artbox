...
export const ToastProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    return (
        <ToastContext.Provider value={{ toast }}>
            {children}
            <div className="fixed top-24 right-6 z-50 flex flex-col gap-2 pointer-events-none items-end">
                <AnimatePresence>
                    {toasts.map((t) => (
                        <motion.div
                            key={t.id}
                            ...
                            layout
                            className={`pointer-events-auto flex items-center gap-3 px-4 py-3 rounded-lg shadow-lg border backdrop-blur-md min-w-[300px] ${t.type === 'success' ? 'bg-zinc-900/95 border-green-500/30 text-green-400' :
